...
class Engine(BaseWikimediaEngine):
    def should_run(self, buffer):
        ...
        size = stdout.decode('ascii').strip().split('x')
    ...
    def shrink(self, buffer, shrink_factor):
        ...
        for i in range(3):
            try:
                ...
                if "does not contain page" in e.args[2].decode('ascii'):
                    # The page is probably out of bounds, try again without
                    # specifying a page
                    source = self.source
                elif "profile" in e.args[2].decode('ascii') and "vips2png: unable to write" in e.args[2].decode('ascii'):
